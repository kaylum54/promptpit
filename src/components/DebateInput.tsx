          ...
          >
            ...
            <div className="relative rounded-xl overflow-hidden" style={{ background: 'var(--bg-base)' }}>
              <input
                ...
                disabled={isLoading}
                className="w-full bg-transparent text-text-primary placeholder-text-muted py-5 px-6 pr-48 text-base font-body outline-none disabled:opacity-50 disabled:cursor-not-allowed transition-shadow duration-200 focus:shadow-[inset_0_0_0_1px_rgba(255,255,255,0.1)]"
                style={{ border: 'none' }}
              />

              {/* Button group inside input */}
              <div className="absolute right-3 top-1/2 -translate-y-1/2 flex items-center gap-2">
                {/* Optimize button - only show when not in debate mode */}
                {!debateComplete && !isLoading && prompt.trim() && (
                  <button
                    ...
                    className={`${modeColor.bg} ${modeColor.hover} text-white p-2.5 rounded-lg disabled:opacity-50 disabled:cursor-not-allowed transition-all duration-200 transform hover:scale-[1.02] active:scale-[0.98]`}
                  >
                    {isOptimizing ? (
                ...
                {debateComplete && onReset ? (
                  <button
                    ...
                    className="bg-white text-[var(--bg-base)] font-semibold text-sm py-3 px-7 rounded-lg transition-all duration-200 transform hover:-translate-y-0.5 hover:shadow-[0_4px_12px_rgba(0,0,0,0.4)] active:translate-y-0 flex items-center gap-2"
                  >
